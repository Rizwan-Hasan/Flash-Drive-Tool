...
from gi.repository import Gtk, GLib, UDisks, XApp
...
# File variables
raw_format_file = appFolder + 'raw_format.py'
raw_write_file = appFolder + 'raw_write.py'
user_interface_file = appFolder + 'window.ui'
...
class MagpieStick:
    def __init__(self, iso_path=None, usb_path=None, filesystem=None, mode=None, debug=False):
        ...
        # get glade tree
        self.gladefile = user_interface_file
        ...
        if mode == "iso":
            ...
            self.wTree.connect_signals(dictCall)
    ...
    def raw_format(self, usb_path, fstype, label):
        if os.geteuid() > 0:
            launcher = 'pkexec'
            self.process = Popen(
                [launcher, '/usr/bin/python3', '-u', raw_format_file, '-d', usb_path, '-f', fstype,
                 '-l', label, '-u', str(os.geteuid()), '-g', str(os.getgid())], shell=False, stdout=PIPE,
                preexec_fn=os.setsid)
        else:
            self.process = Popen(
                ['/usr/bin/python3', '-u', raw_format_file, '-d', usb_path, '-f', fstype, '-l',
    ...
    def format_job_done(self, rc):
        ...
        if rc == 0:
            message = _('Flash drive is formatted successfully.')
            self.logger(message)
            self.success(_('Flash drive is formatted successfully.'))
            return False
        elif rc == 5:
    ...
    def do_write(self, widget):
        ...
        self.logger(_('Flash drive:') + ' ' + self.dev)
    ...
    def raw_write(self, source, target):
        ...
        if os.geteuid() > 0:
            launcher = 'pkexec'
            self.process = Popen(
                [launcher, '/usr/bin/python3', '-u', raw_write_file, '-s', source, '-t', target],
                shell=False, stdout=PIPE, preexec_fn=os.setsid)
        else:
            self.process = Popen(
                ['/usr/bin/python3', '-u', raw_write_file, '-s', source, '-t', target], shell=False,
    ...
    def write_job_done(self, rc):
        if rc == 0:
            ...
            message = _('Not enough space on the Flash drive.')
        elif rc == 4:
...
if __name__ == "__main__":
    ...
    def usage():
        print("Usage: magpiestick [--debug] -m [format|iso]              : mode (format flash drive or burn iso image)")
        print("       magpiestick [--debug] -m iso [-i|--iso] iso_path")
        print("       magpiestick [--debug] -m format [-u|--usb] usb_device ")
    ...
    MagpieStick(iso_path, usb_path, filesystem, mode, debug)
